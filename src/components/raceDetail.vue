<template>
	<div id="wlyall">
		<div id="wlyheader">
		<span class="wlyh1" @click="wlyhandleClick()">《</span>	{{playlist.name}}
		</div>
		<div id="wlynav">
			<div class="wlyn1">
		...
		<div id="wlybody">
			...
			<div class="wlyb2">
				...
				<div class="wlyb24"><span class= 'wlyb241'>报名组别</span><span v-for="(data,index) in datalist" :key="data.id" >{{data.name}}/</span></div>				
			</div>			
		</div>
		<div id="wlysai">
		...
		<div id="wlyfooter" @click="myhandleClick(playlist.id)">
			一键报名
		</div>
	</div>
</template>
...
import url from "./url";
	export default{
		...
		mounted(){
			...
			Indicator.open('加载中...');
			axios.get(`${url.url}/api/detail?id=${this.$route.params.myid}`).then(res=>{
